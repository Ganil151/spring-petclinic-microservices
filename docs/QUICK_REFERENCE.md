...
│   └── dev/templates/ansible_inventory.tftpl  # Inventory template
├── modules/         # Reusable infrastructure modules
├── global/          # Global resources (IAM, Route53)
└── scripts/         # Bare-minimum EC2 user_data (hostname + python3)

ansible/
├── ansible.cfg          # SSH config, pipelining, privilege escalation
├── inventory/
│   └── dynamic_hosts    # ⚡ Auto-generated by Terraform
├── roles/               # One role per tool
│   ├── java/  docker/  awscli/  maven/  kubectl/  helm/
│   ├── jenkins/         # Full Jenkins Master setup
│   ├── sonarqube/       # Kernel tuning + Docker Compose stack
│   └── security_tools/  # Trivy + Checkov
└── playbooks/
    └── install-tools.yml  # 5-play orchestration
```
